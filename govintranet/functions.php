...
$wp_rewrite->add_rewrite_tag('%posts_update%', '([^/]+)', 'status-update=');
 
$wp_rewrite->add_permastruct('news', '/%posts_update%/%news%', false);
...
if($post->post_type != 'news' || empty($permalink) || in_array($post->post_status, array('draft', 'pending', 'auto-draft')))
 
return $permalink;
 
$var1 = has_term('post-format-status', 'post_format', $post_id);
 
if ( $var1 ):
	$var1 = get_the_title($post_id);
	$var1 = sanitize_title($var1);	
else:
	$permalink = str_replace('%posts_update%', 'news', $permalink);
	return $permalink;
endif;
 
if(!$var1) { $var1 = $no_data; }
 
$permalink = str_replace('%posts_update%', 'status-update', $permalink);
...
if ($_SERVER['SERVER_NAME'] == 'intranet2.culture.gov.uk' ) {	
	...
	add_action('init', 'ht_buffer_start');
