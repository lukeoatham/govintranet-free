...
function filter_news($query) {
    if ($query->is_tag && !is_admin()) {
		$query->set('post_type', array('update'));
    }
    return $query;
}; 
...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
		...
		<div class="col-lg-4  col-md-4 col-sm-4 col-lg-offset-1">	
			<?php
				$post_cat = get_the_terms($post->ID,'update-type');
				if ($post_cat){
					$html='';
					...
					if ($cat->term_id){
						...
						$html.= "<span><a class='wptag t".$cat->term_id."' href='".site_url()."/update-type/".$cat->slug."'>".str_replace(" ","&nbsp;",$cat->name)."</a></span> ";
						}
					}	
					if ( $html ){
